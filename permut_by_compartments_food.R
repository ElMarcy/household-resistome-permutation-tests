...
# This script is identical to the other three block of compartment-based randomization scripts (patient/animal/environment/food), with the exception of the randomization section (lines 117-131), which randomizes only the Food compartment blocks 
...
for (i in 1:n_iter) {
  ...
  df_food_random <- table_random %>% filter(sample_type == "Food")           # 5. Take the patient df out
  
  ##### df_ani
  df_animal_random <- df_animal
  df_animal_random$patient_ID_r <- df_animal_random$patient_ID
  ##### df_env
  df_env_random <- df_env
  df_env_random$patient_ID_r <- df_env_random$patient_ID
  ##### df_patient
