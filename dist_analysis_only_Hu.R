...
# This script is identical to the other three compartment-based randomization scripts (patient/animal/environment/food), with the exception of the randomization section (lines 117-131), which randomizes only the Human/Patient compartment
...
for (i in 1:n_iter) {
  ...
  identifiants_patient <- df_patient %>% select(sample_ID, patient_ID, sample_type)     # Keep the metadata columns fixed
  genes_patient_random <- df_patient %>%
    select(-sample_ID, -patient_ID, -sample_type) %>%
    slice_sample(prop = 1)                                                      # Randomize the rows of the 0/1 gene matrix
  df_patient_random <- bind_cols(identifiants_patient, genes_patient_random)       # Reconnect identifiers and randomized data
  ...
  genes_random <- rbind(df_patient_random,df_animal, df_env, df_food)           # Link the randomized tables of the different sample types together
  table_random <- merge(
    table_metadata,
    genes_random,
