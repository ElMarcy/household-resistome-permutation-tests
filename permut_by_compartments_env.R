...
# within households, with randomization of the blocks of a SINGLE compartment of choice 
# (Patient, Animal, Environment or Food).

# This script is identical to the other three block of compartment-based randomization scripts (patient/animal/environment/food), with the exception of the randomization section (lines 117-131), which randomizes only the Environment compartment blocks 
...
distance_jaccard<- vegdist(gene_matrix, method = "jaccard", diag=FALSE, upper=FALSE)
...
## Function to filter by comparison type (includes both directions, for example A-E and E-A)
...
for (i in 1:n_iter) {
  ...
  ##### df_env
  HH_ID <- unique(table$patient_ID)                                          # 1. Retrieve unique identifiers
  HH_ID_random <- sample(HH_ID)                                              # 2. Create a random permutation
  df_HH_ID_random <- data.frame(
    patient_ID   = HH_ID,
    patient_ID_r = HH_ID_random
  )                                                                          # 3. Mapping new to old
  table_random <- table %>%
    left_join(df_HH_ID_random, by = "patient_ID")                            # 4. Add the new column with left_join
  df_env_random <- table_random %>% filter(sample_type == "Environment")     # 5. Take the patient df out
  
  ##### df_ani
  df_animal_random <- df_animal
  df_animal_random$patient_ID_r <- df_animal_random$patient_ID
  ##### df_patient
  df_patient_random <- df_patient
  df_patient_random$patient_ID_r <- df_patient_random$patient_ID
  ##### df_food
  df_food_random <- df_food
  df_food_random$patient_ID_r <- df_food_random$patient_ID
  
  ### Reassemble the table while maintaining the structure of each household in terms of number of H, A, E, and F:
  table_random_all <- rbind(df_patient_random, df_animal_random, df_env_random, df_food_random)

  ### Compute the Jaccard distances for this new, randomized, table.
  #### Build the dataframe of combinations
  ID_r <- expand.grid(table_random_all$sample_ID,table_random_all$sample_ID)
  HH_r <- expand.grid(table_random_all$patient_ID_r,table_random_all$patient_ID_r)
  ST_r <- expand.grid(table_random_all$sample_type,table_random_all$sample_type)
  expanded_df_r <- cbind(ID_r,HH_r,ST_r)
  colnames(expanded_df_r) <- c("ID1", "ID2", "HH1", "HH2", "sample_type1", "sample_type2")         ## Rename columns
  
  #### Extract only the gene matrix and set the sample_ID as rownames.
  gene_matrix_r <- (table_random_all[,4:ncol(table)])                    
  rownames(gene_matrix_r) <- table_random_all$sample_ID
  
  #### Calculation of the Jaccard distance matrix
  distance_jaccard_r <- vegdist(gene_matrix_r, method = "jaccard", diag=FALSE, upper=FALSE)
